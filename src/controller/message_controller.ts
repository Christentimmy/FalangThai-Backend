...
export const messageController = {
  ...
  getMessageHistory: async (req: Request, res: Response) => {
    try {
        ...
        if (!otherUserId) {
            res.status(400).json({ message: "User ID of the other user is required" });
            return;
        }
        ...
        const decryptedMessages = messages.map((msg) => {
            ...
            if (msg.replyToMessage && msg.replyToMessage.multipleImages && msg.replyToMessage.multipleImages.length > 0) {
                msg.replyToMessage.multipleImages = msg.replyToMessage.multipleImages.map((image) => {
                    try {
                        return {
                            ...image,
                            mediaUrl: decrypt(image.mediaUrl, image.mediaIv),
        ...
        console.error(error);
        res.status(500).json({ message: "Failed to retrieve message history", error });
    }
},
  ...
  getListOfActiveFriends: async (userId: string) => {
    try {
      const messages = await Message.find({
        $or: [{ senderId: userId }, { receiverId: userId }],
      }).sort({ timestamp: -1 });
      ...
      messages.forEach((message) => {
        ...
        if (!chatMap.has(otherUserId)) {
          ...
          const chatData = chatMap.get(otherUserId);
          chatData.lastMessage = message.message;
          if (
            message.receiverId.toString() === userId.toString() &&
            message.status === "sent"
          ) {
      ...
      return chatList; // Return the chat list
    } catch (error) {
